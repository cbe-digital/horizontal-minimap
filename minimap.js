...
angular.module('cbeHorizontalMinimap', [])
  ...
  var scale = function() {
      return {
          x: ($window.innerWidth / $parent.width()) * settings.widthRatio,
          y: 1
  ...
  var onResizeHandler = function(e) {
    ...
    var top = $parent.height() * (s.y - 1) / 2 + offsetTop;

    var right = $miniMap.width() * (s.x - 1) / 2  + offsetLeftRight;
    var width = $miniMap.width() * (s.x);
    ...
    var css = {
     ...
     var regionLeft = $miniMap.offset().left * s.x;
     var cssRegion = {
